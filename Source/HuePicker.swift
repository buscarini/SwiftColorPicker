...
public class HuePicker: UIView {
    ...
    func renderBitmap() {
		...
		let provider = CGDataProvider(data: Data(bytes: Array<UInt8>(d)))

//        let provider = CGDataProvider(data: Data(bytes: UnsafePointer<UInt8>(&d), count: d.count * sizeof(UInt8)))
		
		
        let cgimg = CGImage(width: Int(width), height: Int(height), bitsPerComponent: 8, bitsPerPixel: 32, bytesPerRow: Int(width) * Int(sizeof(UInt8) * 4),
